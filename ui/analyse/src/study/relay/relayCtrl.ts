...
export default class RelayCtrl {
  ...
  constructor(
    ...
    this.updateRoundFinishedFromChapters()
  }
  ...
  private updateRoundFinishedFromChapters = () => {
    if (this.round.finished) return

    const chapters = this.study.chapters.list.all()
    if (!chapters.length) return

    if (chapters.every(c => c.status && c.status !== '*')) {
