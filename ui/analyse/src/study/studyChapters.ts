...
export default class StudyChaptersCtrl {
  ...
  setTags = (id: ChapterId, tags: TagArray[]) => {
    ...
    if (!chap || !result) return

    const status = result.replace(/1\/2/g, '½') as StatusStr
    if (chap.status !== status) {
      chap.status = status
      if (chap.playing && status !== '*') chap.playing = false
    }
  }
      ...
      ctrl.chapters.list.all().map((chapter, i) => {
        ...
        return hl(
          ...
          [
            ...
            canContribute && hl('i.act', { attrs: { ...dataIcon(licon.Gear), title: i18n.study.editChapter } }),
          ],
        )
      }),
    ),
